...
const buttonMotion =
  "transition-all duration-200 ease-out hover:scale-[1.03] hover:shadow-md active:scale-[0.96] active:duration-75";
...
export function Button({
  ...
  return (
    <button
      className={`inline-flex items-center justify-center gap-2 rounded-full px-5 py-2.5 text-sm font-semibold disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 disabled:hover:shadow-none ${buttonMotion} ${buttonVariants[variant]} ${className}`}
      {...props}
    />
  );
}
...
export function LinkButton({
  ...
  return (
    <Link
      href={href}
      className={`inline-flex items-center justify-center gap-2 rounded-full px-5 py-2.5 text-sm font-semibold ${buttonMotion} ${buttonVariants[variant]} ${className}`}
    >
      {children}
    </Link>
  );
